<script lang="ts">
	...
	$: yeilds = metadata.additional_metadata[0]?.financial_details?.[0]?.returns?.[0]?.yields || '--';
	$: projectedRent =
		metadata.additional_metadata[0]?.financial_details?.[0]?.rents?.[0]?.projected_rent || '--';
	$: minInvestment =
		metadata.additional_metadata[0]?.financial_details?.[0]?.investment?.[0]?.min_investment ||
		'--';
	$: yearBuilt = metadata.additional_metadata[0]?.property_details?.[0]?.year_built || '--';
</script>
